	...
	<%
		...
		<%
          ...
          for (int count = 1 ; count<=columnCount ; count++ )
          {
            ...
            try
            {
                ...
                { String columnType = columnTypes[count].toUpperCase() ;
                 ...
                 else if (columnType.equals("LONG"))
                 {
                	 ...
                	 catch (Exception e ) { if (is != null) is.close(); columnValue="Error : Unable to Read Value!!!" ; }
                	   
                 } 
		 
                 //--------------If Column Type is CLOB 0r LONG --Or a Varchar with size more than maxDisplayableSize---
                 else if (columnTypes[count].toUpperCase().equals("BLOB") 
            ...
            {  	rs.close();  
          		stmt.close(); 
          		throw new Exception ("Unable to Read Column Number : " + count + " Row Number "+ rownum +" Due to : " + e.getMessage()) ;
          	}
