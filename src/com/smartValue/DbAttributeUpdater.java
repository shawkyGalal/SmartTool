...
import Support.transactions.UserSessionUpdates;

public class DbAttributeUpdater {
	
public static void update(HttpSession session , HttpServletRequest request , HttpServletResponse response, JspWriter out) throws Exception
{
	java.sql.Connection  con = (java.sql.Connection)session.getAttribute("con");
	 ...
	 //--- End of Check if the Record Data is Locked By Approval Cycle or not -----------
	  ...
	  boolean notDate = (!isDate && !isDateTime && ! isHijriDate) ; 
	  ...
	  String updateStatement = "Update " + tableOwner + "." + tableName + " Set " + columnName + "= " + ((notDate)? "'" : "") + newValue + ((notDate)? "'" : "") + " Where rowId = '" + rowIdValue + "'" ;
	...
	{	
				try
			  	{ 
				  	...
				  	if (tableTriggers != null) 
				  		{
				  			tableTriggers.afterUpdate(con , rowIdValue , columnName , newValue) ;
				  		}
				  	...
				  	// Start Auditing the update Operation
				  		...
				  		{
				  			newValueForAuditing = "'" + newValueForAuditing+ "'" ; 
				  		}
				  			...
				  			)
				  	{
				  		...
				  		ModuleServicesContainer msc = Misc.getModuleServiceContainerFromUserSession(session) ; 
				  		...
				  		msc.getListOfUsedTables().remove(dbTable);
				  	}
					...
					out.print("<font color='00FF00'>Ok</font>"); 
				}
				catch (Exception e)
			  	{
					...
					if (con != null) 
					{
						try 
						{
							out.print("Transaction is being rolled back");
							con.rollback(save1);
						} 
