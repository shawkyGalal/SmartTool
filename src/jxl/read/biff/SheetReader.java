...
final class SheetReader
{
  ...
  final void read()
  {
    ...
    while (cont)
    {
      ...
      else if (r.getType() == Type.ROW)
      {
        ...
        if (!rr.isDefaultHeight() || !rr.matchesDefaultFontHeight() ||
            rr.isCollapsed() || 
            rr.isZeroHeight() || rr.hasDefaultFormat())
        {
    ...
    if (!comments.isEmpty())
    {
      logger.warn("note all comments have a corresponding Note record");
    }
  }
  ...
  private void addCellComment(int col, int row, String text)
  {
    ...
    if (c instanceof CellValue)
    {
      CellValue cv = (CellValue) c;
