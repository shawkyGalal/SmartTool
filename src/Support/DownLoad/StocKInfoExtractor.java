...
public void storeDatainDb(Connection con , String tableName) throws Exception 
{
  ...
  //------- Storing The index
  insertStmt = "insert into "+ tableName+" (symbol , price , price_time ) values ('0000' , '"+this.indexValue.trim().replaceAll(",","")+"' , to_date('"+this.dateTimeValue.trim()+"','yyyy/mm/dd hh24:mi:ss') )";
  try{
  ...
  for (int i = 0 ; i< this.symbols.size(); i++)
  {
    ...
    insertStmt = "insert into "+tableName+" (symbol , price , last_trade_vols , no_of_trades , volume_traded ,best_bid_price , best_bid_volume , best_offer_price , best_offer_volumn, price_time) values ("
    ...
    this.run(); // deal with the new values of this company
  }
}
  public void run()
  {
  ...
  public static void main(String[] args) throws Exception
  {
     ...
     String dbServer = "10.16.17.53";
     String dbService = "sadad" ;
     ...
     //Connection con = ConnectionFactory.createConnection("localhost","orcl", "support" , "support");     
     Connection con = ConnectionFactory.createConnection_old(dbServer,dbService, userName , password);     
     int counter = 1;
   
     StocKInfoExtractor sie =  new StocKInfoExtractor();
     ...
     { 
       ...
       sie.storeDatainDb(con , "stock_Info");
       }
       catch (Exception e) 
       { 
        try{ //-- try to reconstruct the connection
        con = ConnectionFactory.createConnection_old(dbServer,dbService, userName , password);    
         } catch (Exception ex){}
    ...
    //--------Popoulating Summary of Last Data in the summary_info table
    System.out.println("======Popoulating Summary of Last Data in the summary_info table===== ");
    
    PreparedStatement prstmt =  con.prepareCall("call populate_summary_info()");
    prstmt.execute();
    
    System.out.println("===End of Popoulating Summary of Last Data in the summary_info table====");
